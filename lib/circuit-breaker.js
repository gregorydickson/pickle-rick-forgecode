...
export class CircuitBreaker {
  ...
  recordIteration(signals, iteration) {
    const s = this._state;
    ...
    if (progress) {
      ...
      s.last_progress_iteration = iteration;
    } else {
    ...
    if (s.state === STATES.CLOSED) {
      if (!progress && s.consecutive_no_progress >= this.config.halfOpenAfter) {
        ...
        this._addHistory(STATES.CLOSED, STATES.HALF_OPEN, iteration);
      }
    } else if (s.state === STATES.HALF_OPEN) {
      if (progress) {
        s.state = STATES.CLOSED;
        this._addHistory(STATES.HALF_OPEN, STATES.CLOSED, iteration);
      } else if (
        ...
        s.state = STATES.OPEN;
        s.total_opens++;
        this._addHistory(STATES.HALF_OPEN, STATES.OPEN, iteration);
      }
    }
